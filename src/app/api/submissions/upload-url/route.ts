...
export async function POST(request: NextRequest) {
  try {
    ...
    const assignment_id = formData.get("assignment_id") as string | null
    const student_id = formData.get("student_id") as string | null

    if (!file || !assignment_id || !student_id) {
      return apiError(new Error("Missing required parameters: file, assignment_id, student_id"), 400)
    }

    // 1. Validate Access — students can only upload for themselves
