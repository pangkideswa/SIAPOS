...
export async function POST(request: NextRequest) {
  try {
    ...
    const { mata_pelajaran, tingkat, kurikulum, topik, jumlah_soal, tipe_soal } = body
    
    if (!mata_pelajaran || !jumlah_soal) {
      return apiError(new Error("Mata pelajaran dan jumlah soal diperlukan"), 400)
    }
    ...
    const typeMapping: Record<string, "PILIHAN_GANDA" | "ESAI"> = {
      ...
      "Essay": "ESAI",
      "Benar / Salah": "PILIHAN_GANDA", // fallback
      "Isian Singkat": "ESAI"
    }
...
Buatkan soal-soal berkualitas tinggi dan mendidik sesuai topik. Hindari pertanyaan yang terlalu umum atau tidak menantang.
${tipe_soal === "Benar / Salah" ? `Setiap soal WAJIB memiliki tepat 2 opsi pilihan yaitu "Benar" dan "Salah", dan pastikan hanya satu opsi yang memiliki 'is_correct' bernilai true.` : tipe_soal === "Pilihan Ganda" ? `Setiap soal WAJIB memiliki tepat 4 opsi pilihan (A, B, C, D), dan pastikan hanya satu opsi yang memiliki 'is_correct' bernilai true.` : tipe_soal === "Isian Singkat" ? `Setiap soal WAJIB menyertakan 'jawaban_benar_esai' berupa satu kata kunci utama atau frasa singkat yang menjadi jawaban paling tepat (maksimal 5 kata).` : ""}
Tingkat kesulitan (kesulitan) dapat bervariasi antara: MUDAH, SEDANG, atau SULIT.`
        ...
        items: {
            ...
            properties: {
                ...
                ...(isPilihanGanda ? {
                    ...
                    jawaban_benar_esai: { type: Type.STRING }
                })
            }
        }
    }
    ...
    for (let i = 0; i < Math.min(generatedQuestions.length, jumlah_soal); i++) {
      ...
      if (isPilihanGanda && Array.isArray(q.pilihan_ganda)) {
          options = q.pilihan_ganda.map((opt: any) => ({
              teks: opt.teks || "Opsi kosong",
              is_correct: !!opt.is_correct
          }))
      } else if (!isPilihanGanda && q.jawaban_benar_esai) {
          options = [{
              teks: q.jawaban_benar_esai,
              is_correct: true
          }]
      }
    ...
    return created({ count: generatedIds.length, ids: generatedIds }, `${generatedIds.length} Soal berhasil di-generate oleh AI`)
  } catch (error) {
