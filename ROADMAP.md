# Roadmap

Roadmap pengembangan SIAPOS — Education Operating System.

---

## Status

| Fase | Modul | Status |
|------|-------|--------|
| 1 | Foundation | ✅ Selesai |
| 2 | Learning Module | ✅ Selesai |
| 3 | Assessment | ⬜ Akan Datang |
| 4 | CBT & Quiz | ⬜ Akan Datang |
| 5 | Communication | ⬜ Akan Datang |
| 6 | Academic Admin | ⬜ Akan Datang |
| 7 | AI Features | ⬜ Akan Datang |

---

## Phase 1 — Foundation ✅

> Sprint 1-2 | v0.1.0 - v0.2.0

- [x] Project setup (Next.js, Laravel, PostgreSQL)
- [x] Authentication (Login, Register, Sanctum)
- [x] Role-based access control (Admin, Guru, Siswa, Wali)
- [x] Dashboard layout (Sidebar, TopBar, BottomNav)
- [x] Landing page
- [x] Manajemen Pengguna
- [x] Master Data Jurusan
- [x] Data Guru
- [x] Data Siswa
- [x] Pengaturan Sekolah

---

## Phase 2 — Learning Module ✅

> Sprint 3 | v0.2.0

- [x] Kelas Mengajar
- [x] Materi Pembelajaran
- [x] Tugas
- [x] Pengumpulan Tugas
- [x] Penilaian
- [x] Dashboard Admin
- [x] Dashboard Guru
- [x] Dashboard Siswa
- [x] QA & Stabilization

---

## Phase 3 — Assessment ⬜

> v0.3.0

- [ ] Quiz Online
- [ ] Bank Soal
- [ ] Analisis Hasil
- [ ] Rekap Nilai

---

## Phase 4 — CBT & Quiz ⬜

> v0.4.0

- [ ] Computer-Based Testing (CBT)
- [ ] Timer & Auto-Submit
- [ ] Anti-Cheating System
- [ ] Hasil Ujian Instan

---

## Phase 5 — Communication ⬜

> v0.5.0

- [ ] Pengumuman Sekolah
- [ ] Kalender Akademik
- [ ] Notifikasi Push
- [ ] Pesan Internal

---

## Phase 6 — Academic Admin ⬜

> v0.6.0

- [ ] Absensi Digital
- [ ] Nilai & Rapor
- [ ] Jadwal Pelajaran
- [ ] Kalender Akademik

---

## Phase 7 — AI Features ⬜

> v1.0.0

- [ ] AI Assistant (Tutor Virtual)
- [ ] AI Quiz Generator
- [ ] AI Analytics (Prediksi Siswa)
- [ ] AI Content Recommendation

---

## Version History

| Versi | Tanggal | Milestone |
|-------|---------|-----------|
| v0.1.0 | Jul 2026 | Initial Setup |
| v0.2.0 | Jul 2026 | Prototype — Learning Module |
| v0.3.0 | TBD | Assessment Module |
| v0.4.0 | TBD | CBT & Quiz |
| v0.5.0 | TBD | Communication |
| v0.6.0 | TBD | Academic Admin |
| v1.0.0 | TBD | Production Release |
